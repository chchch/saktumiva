...
const cleanBlock = (el,type,lang,testonly=false) => {
  ...
  let transliterated = str.toLowerCase();

  if(lang === 'bo' || lang.startsWith('bo-')) {
    if(script === 'tibetan') {
      const ewts = new EwtsConverter({fix_spacing: true, pass_through: true,fix_sloppy: false});
      transliterated = ewts.to_ewts(transliterated
       .replaceAll('ༀ','om̐'))
    ...
    if(!testonly) transliterated = Sanscript.t(transliterated,'wylie','wylish');
  }
  else {
    if(script !== 'iast')
      transliterated = Sanscript.t(transliterated,script,'iast');
      // TODO: just go straight from script -> slpish if !testonly

    if(!testonly) transliterated = Sanscript.t(transliterated,'iast','slpish');
  }
  return transliterated.replaceAll(/\s+/g,' ');
  //return [...filterAll(transliterated.replaceAll(/\s+/g,' '),filters),charset];
};

const findBlockEl = (textel,blockid,idsel,langsel) => {
    if(blockid === null) {
      const lang = textel.closest(`[${langsel}]`)?.getAttribute('xml:lang') ||
                 textel.getRootNode().querySelector(`[${langsel}]`)?.getAttribute('xml:lang') ||
                 'sa';
      return {block: textel, lang: lang};
    }

    const blockel = textel.querySelector(`*[${idsel}="${blockid}"], *[corresp~="#${blockid}"]`);
    ...
    const clone = (blockel.querySelector('*[type="edition"]') || blockel).cloneNode(true);
    
    return {block: clone, lang: lang};
};
...
const preProcess = (block, texts, opts) => {
  ...
  for(const text of texts) {
    ...
    const res = findBlockEl(textel,block,idsel,langsel);
    if(!res) continue;
    const {block: blockel, lang: lang}  = res;
    ...
    if(type) {
      if( (type === 'ac' || type == 'pc') ) {
        ...
        if(!acpc) {
          if(type === 'ac') {
              const clean = cleanBlock(blockel,'lem',lang,testonly);
              ret.push({siglum: par, text: clean, lang: lang});
          }
          continue;
        }
    }
    ...
    const clean = text.text.collectives ?
      cleanBlock(blockel,[type,...text.text.collectives],lang,testonly) :
      cleanBlock(blockel,type,lang,testonly);
    ret.push({siglum: siglum, text: clean, lang: lang});
  }

  if(!testonly)
    filterTexts(ret,splitfunc,selectedfilters);
  return ret;
};

const filterTexts = (arr, splitfunc, selectedfilters) => {
  for(const obj of arr) {
    const [filtered,filters] = filterAll(obj.text,selectedfilters);
    const charset = obj.lang === 'bo' || obj.lang.startsWith('bo-') ? wylish : slpish;
    const split = splitfunc(filtered,charset);
    obj.text = split;
    obj.filters = filters;
  }
};
...
const cleanup1 = arr => {
    ...
    if(arr[0] !== '' && arr[0].trim() === '') {
      ...
      if(firstnonempty !== null) {
        arr[firstnonempty] = arr[0] + arr[firstnonempty];
        arr[0] = '';
      }
    }
    */
...
const processFile = (teixml, filename, state, idsel = '*|id') => {
    ...
    if(!overallsiglum && textsigla.length === 0) {
        ...
        overallsiglum = filename.split('/').pop();
    }
    ...
    for(const el of els) {
        ...
        const corresps =  el.getAttribute('corresp')?.split(/\s+/).map(c => c.replace(/^#/,''));
        if(corresps && corresps.length > 0)
          for(const corresp of corresps)
            state.allblocks.add(corresp);
    }
    return warnings;
};
...
export { processFile, preProcess, filterTexts, postProcess, groupBySpace, detectScript, findSplitfunc, cleanup1, cleanup2, makeWitList, untransliterate };
