...
const scriptandfilter = (el,type,filters,langsel) => {
    ...
    const lang = el.closest(`[${langsel}]`)?.getAttribute('xml:lang');
    ...
    return [...filterAll(transliterated.replaceAll(/\s+/g,' '),filters),lang,charset];
};
    ...
    for(const text of texts) {
        ...
        if(type) {
            if( (type === 'ac' || type == 'pc') ) {
                ...
                if(!acpc) {
                    if(type === 'ac') {
                        const [clean,filters,lang,charset] = scriptandfilter(blockel,'lem',selectedfilters,langsel);
        ...
        const [clean,filters,lang,charset] = scriptandfilter(blockel,type,selectedfilters,langsel);
...
const processFile = (teixml, filename, state, idsel = '*|id') => {
    ...
    // merge texts with the same siglum
