import Transliterate from './transliterate-min.mjs';
...
const latexCleanup = el => {
  const walker = document.createTreeWalker(el,NodeFilter.SHOW_ALL);
  let cur = walker.nextNode();
  while(cur) {
      if(cur.nodeType === 3) {
          cur.data = cur.data.replaceAll('&','{\\ampersand}').replaceAll('_','\\_');
      }
      cur = walker.nextNode();
  }
};
...
const processPostOptions = (doc,exportRoot) => {
    if(exportRoot.querySelector('#export-underline').checked)
        markUnderlines(doc);
    if(!exportRoot.querySelector('#export-line-breaks').checked)
        for(const lb of doc.querySelectorAll('lb'))
            lb.remove();
    if(!exportRoot.querySelector('#export-page-breaks').checked)
