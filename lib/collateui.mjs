...
const doCollate = async (xmldoc,htmldoc,alignments,popup,opts) => {
    ...
    await previewBlocks(htmldoc,xmldoc,blocklist,opts);
    button.style.display = 'unset';
    spinner.style.display = 'none';
    popup.style.display = 'none';
    return blocklist;
...
const previewBlocks = async (htmldoc,xmldoc,blocklist,opts) => {
    let docclone = xmldoc;
    if(blocklist.length === 1) {
      // TODO: more than just one?
      docclone = docclone.cloneNode(true);
      const text = docclone.querySelector('text');
      // TODO: if there is more than one text element
      const blockel = text.querySelector(`[*|id="${blocklist[0]}"]`);
      const parallel = blockel.closest('div[rend="parallel"]');
      if(parallel)
        text.replaceChildren(parallel);
      else
        text.replaceChildren(blockel);
    }
    const newDoc = opts.mode === 'serverless' ? 
        await previewDoc(docclone) :
        await previewDoc(docclone,'./edition.xsl');
...
*/
export { doCollate, previewBlocks };
