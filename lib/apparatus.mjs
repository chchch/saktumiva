...
const cleanBlock = (blockid,idsel,wit) => {
	...
	for(const nobreak of block.querySelectorAll('lb[break="no"]')) {
...
const collectWitGroups = listWit => {
	...
	for(const group of listWit.querySelectorAll('witness[n="collective"]')) {
		...
		const wits = [...group.querySelectorAll('witness')].filter(w => !(w.getAttribute('n') === 'collective')).map(w => w.getAttribute('xml:id'));
