...
const scorechar = ({match = 1, mismatch = -1, missing = '‡'}) => (s1,i,s2,j) => {
    ...
    const b = s2[j];
    if(a === b) return match;
    if(a == missing || b === missing) return 0; // TODO: specify this penalty
    //if(a === '' && b === ' ') return 1;
    //if(a === ' ' && b === '') return 1;
    //if(a === '' || b === '') return -2;
    //if(a === ' ' || b === ' ') return -2;
...
const scorearr = ({match = 1, mismatch = -1, gap_open = 0, gap_extend = -2, missing = '‡'}) => (s1,i,s2,j) => {
    const a = s1[i];
    const b = s2[j];
    ...
    const score = a.length === 1 && b.length === 1 ?
        scorechar({match: match, mismatch: mismatch, missing: missing})(a,0,b,0) :
...
const affineAlign = (s1arr,
    ...
    for(let i=1; i<s1len+1; i++) {
        for(let j=1; j<s2len+1; j++) {
            ...
            if( maxval === ulscore) direc[i][j] = UL;
            else if( maxval === igapmax) direc[i][j] = UP;
