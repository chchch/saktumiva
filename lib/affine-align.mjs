...
const affineAlign = (s1arr,
    ...
    // initialize matrix
    const ArrayType = estimateSize(s1len+1,s2len+1,config);
    ...
    for(let i=1; i<s1len+1; i++) {
        for(let j=1; j<s2len+1; j++) {
            ...
            */
            const igapmax = Math.max(igapopen,previgap) + config.gap.extend;
            ...
            */
            const jgapmax = Math.max(jgapopen,prevjgap) + config.gap.extend;
            /* 
...
const alignAlign = (p1arr,
    ...
    // initialize matrix
    const ArrayType = estimateSize(p1len+1,p2len+1,config);
    ...
    for(let i=1; i<p1len+1; i++) {
        for(let j=1; j<p2len+1; j++) {
            let ulscore = mat.getValue(i-1,j-1) + config.scorefn(p1arr,i-1,p2arr,j-1);
            // no gap opening penalty at the bottom row
