        ...
        if(typeof inputFormat === "object"){
            ...
            if(inputFormat.xml){ // default xml format is phyloXML
                if(!inputFormat.fileSource){
                    xj = XMLObjectifier.textToXML(inputFormat.xml);    // assume we need to clean it up
                } else {
                    xj = inputFormat.xml;
                }
                ...
                if(!inputFormat.fileSource){
                    xj = XMLObjectifier.textToXML(inputFormat.phyloxml);    // assume we need to clean it up
                } else {
                    xj = inputFormat.phyloxml;
                }
                xj = XMLObjectifier.xmlToJSON(xj);
                dataObject = new Smits.PhyloCanvas.PhyloxmlParse(xj);
            } else if(inputFormat.nexml){
                if(!inputFormat.fileSource){
                    xj = XMLObjectifier.textToXML(inputFormat.nexml);    // assume we need to clean it up
                } else {
                    xj = inputFormat.nexml;
                }
                xj = XMLObjectifier.xmlToJSON(xj);
        ...
        recursiveParse = function(clade, parentNode){
            ...
            if(clade.annotation){
                ...
                if(clade.annotation[0] && clade.annotation[0].img){
                    for(let i in clade.annotation[0].img){
...
Smits.PhyloCanvas.NexmlParse.prototype = {
    ...
    return function(inputFormat){
        ...
        if(inputFormat.tree && jsonString.trees[0] && jsonString.trees[0].tree[(inputFormat.tree-1)]){
            ...
            for(let i = 0; i < jsonString.trees.tree.edge.length; i++){
                nexEdges.push(jsonString.trees.tree.edge[i]['@attributes']);
            }
            for(let i = 0; i < jsonString.trees.tree.node.length; i++){
        ...
        for(let i = 0; i < nexNodes.length; i++){
            if(nexNodes[i].root && nexNodes[i].root == "true"){
                root = nexNodes[i];
            }
        }
...
Smits.PhyloCanvas.Render.SVG.prototype = {
    ...
    render : function(){
        ...
        for (var i = 0; i < instructs.length; i++) {
            ...
            if(instructs[i].type == 'line'){
                ...
                /*path = */this.svg.path(instructs[i].path).attr(instructs[i].attr);
            } else if(instructs[i].type == 'circle'){
                /*path = */this.svg.circle(instructs[i].x, instructs[i].y, instructs[i].radius).attr({
                    "stroke": 'black',
                    "fill":   'black'
                });
            } else {
    ...
    var svg,
        ...
        var recursiveCalculateNodePositions = function (node, positionX){
            //console.log(" ");
            // console.log("recursiveCalculateNodePositions for node: " + node.name);
            var nodeCoords = [], x1,x2,y1,y2,r,g,b;
        ...
        renderBarChart = function(x, groupName, params){
            ...
            for(let i = 0; i < labelsHold.length; i++){
                var node = labelsHold[i];
                svg.draw(
                    new Smits.PhyloCanvas.Render.Path(
                        rectLinePathArray(
    ...
    return function(sSvg, dataObject){
        ...
        /* fix for extra padding with multiple trees */
        firstBranch = true;
        ...
        labelsHold = [];
        if(typeof bgLabelsHold !== 'undefined') bgLabelsHold = [];
...
Smits.PhyloCanvas.Render.Phylogram.prototype = {
    ...
    function renderBackground(){
        ...
        if(bgLabelsHold.length > 0){
            ...
            for(let i = 0; i < bgLabelsHold.length; i++){
    ...
    var _self = {
        xmlToJSON: function(xdoc) {
            try {
                ...
                const setAttributes = function(obj, node) {
                        ...
                        activate: function() {
                            ...
                            if(!!nodes) {
                                ...
                                nodes.SortByAttribute = function(col, dir) {
                                    if(!!nodes && nodes.length > 0) {
                                        const getValue = function(pair, idx) {
                                            var out = pair[idx];
                                ...
                                nodes.SortByNode = function(node, dir) {
                                    if(!!nodes && nodes.length > 0) {
                                        const getValue = function(pair, node) {
                ...
                //Set collections
                const setHelpers = function(grpObj) {
                ...
                //Set Object Nodes
                const setObjects = function(obj, node) {
